...
void DynaMsgConnection::disconnectFromHost()
{
...
void DynaMsgConnection::sendAuthRequest(QString name,QString pass)
{
	m_socket->write(m_parser->generateAuthRequest(name,pass));
	m_socket->flush();
}
void DynaMsgConnection::sendAuthReply(bool valid)
{
	m_socket->write(m_parser->generateAuthReply(valid,0));
	m_socket->flush();
